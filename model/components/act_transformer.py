...
class Transformer(nn.Module):
    ...
    def forward(self, src, mask, query_embed, pos_embed, latent_input=None, proprio_input=None, additional_pos_embed=None):
        ...
        cond_proj = self.linear_proj(cond).unsqueeze(0)
        src = torch.concat([src, cond_proj], dim=0)
        pos_cond = torch.ones_like(pos_embed[0]).unsqueeze(0).to(pos_embed.device, pos_embed.dtype)
        pos_embed = torch.concat([pos_embed, pos_cond], dim=0)
